...
public class ConsoleAdminSession implements ISession {
    ...
    private void changeAccessPeriodMenu() {
        ...
        do {
            //prompts user to re-input start date so long as the format is invalid
            String startDateStr;
            ...
            //prompts user to re-input end date so long as the format is invalid
            String endDateStr;
    ...
    private void addStudentMenu() {
        ...
        //adds the student and his/her information into the user database
        //and lists all students in the database
        try {
            Student newStudent = Factory.createStudent(name, school, gender, nationality, maxAUs);
            IReadWriteUserDataAccessObject userDataAccessObject = Factory.getTextUserDataAccessObject(this);
            //calls userDataAccessObject to write the student object into the user database
            ...
            //calls userDataAccessObject to retrieve information of all students
    ...
    private void checkIndexVacanciesMenu() {
        ...
        try {
            //retrieves all course codes via courseDataAccessObject
            //and prompts the user to select among those courses
            ...
            //retrieves the indexes for the given course code
            //and prompts user to select among those indexes
    ...
    private void addUpdateDropCourseMenu() {
        ...
        try {
            ...
            if (selectedCourse == null) {
                ...
                //prompts user to input a new course code so long as it is not in the valid format
                do {
                    ...
                    if (validCourseCode) {
                        //checks if the course input is an existing course
                        selectedCourse = courseDataAccessObject.getCourse(selectedCourseCode);
                        if (selectedCourse != null) {
                            _terminal.getProperties().setPromptColor("red");
        ...
        if (selectedCourse == null) {
            ...
            //ensures that courseName, school and AUs inputs are valid
            ...
            do {
                ...
                //adds the lecture day and timing to the Lecture Timings hashtable with lecture day as key
                lectureTiming = getValidTimeInput("lecture");
                lectureTimings.put(lectureDay, lectureTiming);

                //prompts user to indicate Y/y to continue adding and N/n to stop adding
                contAdd = _textIO.newBooleanInputReader().
                        read("do you wish to continue adding a lecture session?");
            } while (contAdd);

            //prompts user to select among valid venues
            ...
            //lists information of all courses retrieved via courseDataAccessOject after course is successfully added
            try {
                IReadWriteCourseDataAccessObject courseDataAccessObject = Factory.getTextCourseDataAccessObject(this);
            ...
            _textIO.newStringInputReader()
                    .withDefaultValue(" ").
                    read("press enter to continue");
        } else {  //prompts user to udpate course-level information
            int option;
            do {
                ...
                if (selectedCourse.getListOfIndexNumbers().size() == 0) {
                    ...
                    _terminal.getProperties().setPromptColor("white");
                    option = 5;
                } else { //prints course information available
    ...
    private Course addUpdateDropIndexMenu(Course course, int index) {
        ...
        if (existingIndex == null) {
            ...
            //Prompt user to enter selection of maximum class size and read it
        ...
        do {
            ...
            switch (option) {
                case 1 -> {
                    ...
                    //Prompt user to enter selection of day of tutorial and read it
                    ...
                    if (existingIndex.getTutorialVenue() == null) {
                        ...
                        _terminal.getProperties().setPromptColor("white");
                        //Prompt user to enter selection of tutorial venu and read it
                ...
                case 2 -> {
                    ...
                    if (existingIndex.getLaboratoryVenue() == null) {
                        ...
                        _terminal.getProperties().setPromptColor("white");
                        //Prompt user to enter selection of lab venue and read it
                ...
                case 3 -> {
                    ...
                    //Prompt user to enter selection of venue and read it
                    Venue tutorialVenue = _textIO.newEnumInputReader(Venue.class)
                    ...
                    _terminal.getProperties().setPromptColor("white");
                    //Checks if tutorial timing exist
                    if (existingIndex.getTutorialTimings() == null) {
                        ...
                        //Validate input
                        List<LocalTime> sessionTiming = getValidTimeInput("tutorial");
                        //Put the day and timing into newTutorialTiming  HashTable
                        newTutorialTimings.put(sessionDay, sessionTiming);
                        //Set new tutorial timing
                        existingIndex.setTutorialTimings(newTutorialTimings);
                ...
                case 6 -> {
                    try {
                        ...
                        //index group deleted from course
    ...
    private void printStudentListByIndexMenu() {
        try {
            IReadWriteCourseDataAccessObject courseDataAccessObject = Factory.getTextCourseDataAccessObject(this);
            //Prompt user to enter selection of course he/she wants to print and read it
            String courseCode = _textIO.newStringInputReader()
                    .withNumberedPossibleValues(courseDataAccessObject.getAllCourseCodes())
                    .read("print student list by index");
            Course course = courseDataAccessObject.getCourse(courseCode);

            //Prompt user to enter selection of index he/she wants to print and read it
    ...
    private void printStudentListByCourseMenu() {
        try {
            ...
            //Prompts user to enter selection of the course he/she wants to print and read it
            Course course = courseDataAccessObject.getCourse(courseCode);
            _terminal.getProperties().setPromptColor(Color.GREEN);
            _terminal.println("Successfully retrieved student list");
    ...
    private void loginAsStudentMenu() {
        try {
            ...
            //Prompt user to enter student username and read it
            String username = _textIO.newStringInputReader()
                    .read("Enter Username:");
            //Prompt user to enter student password and read it
    ...
    private String getStudentListByIndexString(Index index) {
        ...
        try {
            ...
            //Append to StringBuilder object base on the following conditions
    ...
    private List<LocalTime> getValidTimeInput(String sessionType) {
        /**
         * List of LocalTime that represents start and end time 
         */
        ...
        String startTime;
        /**
         * School start time in String
         */
        String schoolStartTime = "07:30";
        /**
         * School end time in String
         */
        String schoolEndTime = "21:30";
        /**
         * Maximum duration in int
         */
        int maxDuration = 4;
        //Prompt user to enter the duration of the session and reads it
